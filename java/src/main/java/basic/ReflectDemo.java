...
//        getClassOther();
    ...
    private static void getClassField() throws Exception {
        // 通过反射获取类的属性
        Class<?> c = Class.forName("basic.Person");
//        Field[] fields = c.getFields();  // 只能获取public修饰的属性
    ...
    private static void getClassOther() throws Exception {
        ...
        for (Type genericInterface : genericInterfaces) {
            System.out.println(genericInterface.getTypeName());  // java.io.Serializable  java.lang.Comparable<basic.Person>
        }
    ...
    private static void test01() throws Exception {
        // 通过反射创建类的对象
        Class<?> c = Class.forName("basic.Person");
        System.out.println(c);  // class basic.Person
        ...
        // 暴力破解,直接访问会报错 java.lang.IllegalAccessException: Class basic.ReflectDemo can not access a member of class basic.Person with modifiers "private"
    ...
    private static void test02() throws Exception {
        ...
        // 关联输入流,从文件读取属性
        FileReader fr = new FileReader(new File("java/input/pci.properties"));
        prop.load(fr);
        // 遍历集合
        Set<String> keys = prop.stringPropertyNames();
        for (String key : keys) {
            String value = prop.getProperty(key);
            System.out.println(value);  // basic.NetCard, basic.SoundCard
            // 通过反射加载类
