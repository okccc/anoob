 ...
 *
 * 缓存
 * 为了减轻数据库压力,通常会将高频访问的数据放到redis做缓存,请求过来时先查缓存,缓存没有再去查数据库并将查询结果添加到缓存
 *
 * 缓存穿透
 * 某个热点key失效,请求直接怼到数据库,此时redis形同虚设相当于不存在,仿佛被击穿
 *
 * 缓存雪崩
 * 大量热点key在同一时间失效,高并发场景下所有请求怼到数据库导致崩溃,尽量将缓存key的失效时间均匀错开
 *
 * 海量数据去重
