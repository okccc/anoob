 ...
 *
 * Checkpoint
 * 检查点是一种故障恢复机制,类似游戏存档,会定期拷贝当前状态,分界线是所有算子任务都处理完一条相同输入数据时,barrier用来对齐算子间的检查点
 * "检查"是针对故障恢复的结果而言,从checkpoint重放数据的计算结果应该和发生故障前完全一致,所以也叫"一致性检查点",默认只保存最近一次检查点
 * 检查点同步实现：暂停应用,保存状态到检查点,再重新恢复应用(sparkStreaming)
 * 检查点异步实现：基于Chandy-Lamport分布式异步快照算法,将检查点的保存和数据处理分开,不暂停应用(flink)
 * 检查点相关的metrics也是诊断flink应用健康状态的重要指标,成功且耗时较短的检查点表明作业运行状况良好,没有出现异常或反压
 * 由于检查点和反压的耦合,反压也会反作用于检查点,导致检查点的种种问题,于是flink1.11引入Unaligned Checkpoint进行解耦优化
 *
