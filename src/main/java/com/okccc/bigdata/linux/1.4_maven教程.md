...
# 4.maven依赖原则：最短路径者优先,路径相同时先声明者优先
# scope控制依赖jar包使用范围
...
```shell script
# 查看工程依赖树
mvn dependency:tree  # -Dverbose参数会把被忽略的jar包,即相同jar包的不同版本也列出来,-Dincludes参数只显示指定的jar包
mvn dependency:tree -Dverbose -Dincludes=commons-collections  # 查看commons-collections包的冲突情况
...
这里面3.2.2版本jar包的被忽略了,maven解析依赖出现jar包版本冲突时会选择路径更短声明更早的那个,该命令可以查看jar包的引入路径和冲突情况
