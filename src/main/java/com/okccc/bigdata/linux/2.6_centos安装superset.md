...
# 安装superset,-i指定镜像,下载失败可以更换下面那个镜像
(superset)[root@cdh1 ~]$ pip install apache-superset -i https://pypi.douban.com/simple/
...
# 启动superset,--workers进程数,--timeout超时重启,--bind绑定地址,--daemon后台运行
(superset)[root@cdh1 ~]$ gunicorn --workers 5 --timeout 120 --bind 10.18.3.24:8787 "superset.app:create_app()" --daemon
# 启动报错
...
superset_start(){
    ...
    if [[ $? -eq 0 ]]; then
        conda activate superset; gunicorn --workers 5 --timeout 120 --bind 10.18.3.24:8787 'superset.app:create_app()' --daemon
    else
        echo "superset 正在运行"
    fi
}
