...
sudo -u hdfs hdfs dfs -chwon root /user/flume
...
# cdh重启yarn后ResourceManager没有Active,两个节点都是Standby
重启yarn时会尝试恢复之前所有任务导致内存溢出,进入zk客户端查看 ls /rmstore/ZKRMStateRoot/RMAppRoot发现大量application,直接清空目录重启yarn
```