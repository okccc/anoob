...
public class ProducerDemo {
    public static void main(String[] args) throws InterruptedException {
        ...
        // 可选参数
        prop.put(ProducerConfig.ACKS_CONFIG, "all");                    // ack可靠性级别 0/1/-1(all)
        prop.put(ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG, true);       // 开启幂等性机制,配合ack=-1确保生产者exactly once
        prop.put(ProducerConfig.RETRIES_CONFIG, 1);                     // 重试次数
        prop.put(ProducerConfig.BATCH_SIZE_CONFIG, 1024*16);            // 批次大小,当数据累积到该数值后sender线程才会发送到kafka,可以控制生产者吞吐量
        prop.put(ProducerConfig.LINGER_MS_CONFIG, 10);                  // 等待时间,如果数据迟迟未达到batch.size大小,sender线程等待该时间后就发送数据
        prop.put(ProducerConfig.MAX_REQUEST_SIZE_CONFIG, 1024*1024*5);  // 生产者往kafka批量发送请求的最大字节数,默认1M
        prop.put(ProducerConfig.BUFFER_MEMORY_CONFIG, 1024*1024*32);    // 缓冲区大小
        // 添加拦截器集合(可选)
