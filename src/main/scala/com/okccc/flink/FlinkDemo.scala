...
object FlinkDemo {
  def main(args: Array[String]): Unit = {
     ...
     *
     * flink特点
     * 1.低延迟(spark秒级flink毫秒级)&高吞吐,完美适合流式数据场景,批流结合,批处理和流处理共用一个引擎
     * 2.支持时间语义,EventTime + Watermark可以处理迟到和乱序数据保证结果准确,sparkStreaming没有时间语义概念,无法处理延迟数据
     * 3.支持exactly-once精准一次性消费
     * 4.支持有状态的计算,可以将中间结果保存在内存或文件中,提高效率和容错性
     * 5.支持窗口操作,各种灵活的窗口及触发条件可以处理复杂的业务场景
     * flink在jvm中实现了自己的内存管理,应用可以超出主内存的大小限制,承受更小的垃圾回收开销
     *
     * API
     ...
     *
     * 时间语义(time)
     * 发生顺序：事件创建时间(EventTime) - 数据进入flink时间(IngestionTime) - 执行算子的本地机器系统时间(ProcessingTime)
     ...
     * 窗口计算对于EventTime内的延迟数据不能无限期等下去,必须要有某种机制保证到达特定时间(水位线)后就触发窗口计算,该机制就是watermark
     ...
     * 延迟时间t的设定：太大会导致等待时间过久实时性很差,太小实时性高了但是会漏数据导致结果不准确,数据的乱序程度应该是符合正态分布的,可以先设置
     * 一个很小的延迟时间hold住大部分延迟数据,剩下少量延迟太久的数据通过allowLateness和sideOutput处理,这样就能同时兼顾结果准确性和实时性
...
//    val inputStream: DataStream[String] = env.fromElements("hello hadoop", "hello spark", "hello flink")

    // 扁平化
    val flatMapStream: DataStream[String] = inputStream.flatMap((s: String) => s.split(" "))

    // 映射
    val mapStream: DataStream[WordCount] = flatMapStream.map((s: String) => WordCount(s, 1L))
    val mapStream2: DataStream[(String, Long)] = flatMapStream.map((s: String) => (s, 1L))

    // 分组
    val keyedStream: KeyedStream[WordCount, String] = mapStream.keyBy((wc: WordCount) => wc.word)
    val keyedStream2: KeyedStream[(String, Long), String] = mapStream2.keyBy((t: (String, Long)) => t._1)

    // 聚合
    val result: DataStream[WordCount] = keyedStream.reduce((wc1: WordCount, wc2: WordCount) => WordCount(wc1.word, wc1.count + wc2.count))
    val result2: DataStream[(String, Long)] = keyedStream2.reduce((t1: (String, Long), t2: (String, Long)) => (t1._1, t1._2 + t2._2))
...
//    val socketStream: DataStream[String] = env.socketTextStream(host, port)
    ...
    val kafkaStream: DataStream[String] = env.addSource(
      // 使用实现了SourceFunction接口的flink-kafka消费者api,需指定topic,反序列化器,kafka地址和消费者组
      new FlinkKafkaConsumer[String]("nginx", new SimpleStringSchema(), prop)
    )
    ...
    val mapStream: DataStream[UserBehavior] = kafkaStream.map((data: String) => {
      ...
      UserBehavior(words(0), words(1), words(2), words(3), words(4).toLong)
    })
...
//    mapStream.addSink(new FlinkKafkaProducer[String]("localhost:9092", "flink-sink", new SimpleStringSchema()))
...
class MyJdbcSink extends RichSinkFunction[UserBehavior] {
  ...
  override def invoke(value: UserBehavior, context: Context): Unit = {
    ...
    ps.setInt(3, value.categoryId.toInt)
...
class MyRedisMapper extends RedisMapper[UserBehavior] {
  ...
  // 从对象中提取field
  override def getKeyFromData(data: UserBehavior): String = data.userId

  // 从对象中提取value
  override def getValueFromData(data: UserBehavior): String = data.itemId
}
