...
object S01_RDD {
  def main(args: Array[String]): Unit = {
     ...
     * 序列化：存储和网络传输是通过IO流的字节序列实现,序列化就是将内存中的对象转换成字节,便于持久化或网络传输,再次使用需将字节反序列化为对象
    ...
    val lines: RDD[String] = sc.textFile("scala/input/aaa.txt", minPartitions=2)
...
//    sc.setCheckpointDir("hdfs://cdh1:9000/checkpoint")
     ...
     * |  scala/input/aaa.txt MapPartitionsRDD[69] at textFile at S01_RDD.scala:215 [Memory Deserialized 1x Replicated]
     * |  scala/input/aaa.txt HadoopRDD[68] at textFile at S01_RDD.scala:215 [Memory Deserialized 1x Replicated]
     */
     ...
     * |  scala/input/aaa.txt MapPartitionsRDD[69] at textFile at S01_RDD.scala:215 []
     * |  scala/input/aaa.txt HadoopRDD[68] at textFile at S01_RDD.scala:215 []
     *
     * checkpoint之后
     * (2) ShuffledRDD[72] at reduceByKey at S01_RDD.scala:234 []
    ...
    // 读取文件
    val file: RDD[String] = sc.textFile("scala/input/log.txt")
    // 结构转换
