...
object S05_group {
  def main(args: Array[String]): Unit = {
    ...
    // 按元素奇偶性分组
    val intToList1: Map[Int, List[Int]] = ints.groupBy((i: Int) => {
      i % 2
    })
    println("res1=" + intToList1) // Map(1 -> List(1, 3), 0 -> List(2, 4))
    intToList1.foreach((t: (Int, List[Int])) => {
      t._1 + "=" + t._2
    })
    // 按字符串首字母分组
    val strings: List[String] = List("aa", "ab", "cc", "cd")
    val stringToList: Map[String, List[String]] = strings.groupBy((i: String) => {
      i.substring(0, 1)
    })
    println("res2=" + stringToList) // Map(a -> List(aa, ab), c -> List(cc, cd))
    stringToList.foreach((t: (String, List[String])) => {
      println(t._1 + "=" + t._2)
    })
    ...
    val ints_sort_desc: List[Int] = ints.sortWith((x: Int, y: Int) => {
      x > y
    }).take(3)
    println("res4=" + ints_sort_desc) // List(4, 3, 3)
    val strings_sort_desc: List[String] = strings.sortWith((x: String, y: String) => {
    ...
    // 再映射
    val wordToInt: Map[String, Int] = wordToList.map((t: (String, List[String])) => {
      (t._1, t._2.size)
    })
    println(wordToInt) // Map(spark -> 1, scala -> 2, java -> 3, python -> 2)
    // Map集合是无序的没有排序方法,需转换成List集合再排序
    val result: List[(String, Int)] = wordToInt.toList.sortWith((x: (String, Int), y: (String, Int)) => {
      x._2 > y._2
    }).take(3)
    ...
    // 分组
    val groupWordMap: Map[String, List[(String, Int)]] = flatMapList.groupBy((t: (String, Int)) => {
      t._1
    })
    ...
    val wordSumMap: Map[String, Int] = groupWordMap.mapValues((l: List[(String, Int)]) => {
      l.map((t: (String, Int)) => {
        t._2
      }).sum
    })
    ...
    // 排序
    val wordCount: List[(String, Int)] = wordSumMap.toList.sortWith((x: (String, Int), y: (String, Int)) => {
      x._2 > y._2
    }).take(3)
