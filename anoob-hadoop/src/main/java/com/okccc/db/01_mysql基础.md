...
# 3.索引数据结构
# Hash索引只能等值查询,B+树索引可以范围查询
# 查询数据库访问磁盘的次数由树的层数决定,二叉树只有左右两个子节点而B+树可以有多个子节点,降低树的高度可以减少磁盘I/O
# B树非叶子节点既存数据也存键值,B+树非叶子节点不存数据只存键值,这样就能存储更多的键值,节点的子节点树就会更大,树就会更矮更胖,减少磁盘I/O
