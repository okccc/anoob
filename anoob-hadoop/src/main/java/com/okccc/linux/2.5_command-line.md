...
-Dtaskmanager.numberOfTaskSlots=3 \         # 单个TaskManager的slot数,比如并行度为5则需要5/3≈2个TaskManager即6个任务槽,会向yarn动态申请资源
-Dyarn.containers.vcores=3 \                # 单个容器的vcore数,默认=taskmanager.numberOfTaskSlots(可选)
-Dclassloader.resolve-order=parent-first \  # 控制类加载顺序,优先加载Flink内置依赖org.apache.flink.xxx,避免用户jar依赖冲突
