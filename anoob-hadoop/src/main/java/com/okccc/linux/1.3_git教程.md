...
# 修改上次提交备注
git commit --amend -m "..."
# 修改历史提交备注,先rebase进入编辑模式,找到想要修改备注的commit,将pick改为reword后修改备注,强制推送到远程仓库
git rebase -i <commit-id> && git push -f
# 使用rebase命令时工作目录不能有未暂存的更改,可以先stash将这些更改暂时存储起来,等rebase结束后再恢复
...
### branch
```shell script
# 查看本地分支(其中绿色带*是当前分支)/远程分支(红色)/所有分支
git branch (-r/-a)
# 创建分支并切换分支
...
- 22端口是基于SSH协议容易被企业屏蔽,443端口是基于HTTPS协议几乎通用
vim ~/.ssh/config
Host github.com
...
# remote
# 将github工程克隆到本地,默认master分支
git clone git@github.com:okccc/python.git
# 首次切换分支时先创建本地分支并关联远程,后续可以直接切换
git checkout -b dev origin/dev | git checkout dev  # 传统方式
git switch -c dev origin/dev | git switch dev  # Git2.23+推荐
# 查看本地分支是否已关联远程分支,[origin/dev]表示已关联,后续fetch/pull/push时就不用再指定远程主机和分支
git branch -vv
