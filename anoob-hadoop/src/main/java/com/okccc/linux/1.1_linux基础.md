...
deploy:x:7012:7012:deploy user:/home/deploy:/bin/bash
...
[root@cdh1 ~]$ who
# 显示机器所有信息
[root@cdh1 ~]$ uname -a, --all
# 显示机器处理架构
[root@cdh1 ~]$ uname -m, --machine
# 显示机器内核版本
[root@cdh1 ~]$ uname -r, --kernel-release
# 查看操作系统
...
# df (disk free)
[root@cdh1 ~]$ df -hT  # 显示系统盘类型,-T, --print-type
...
[root@cdh1 ~]$ df -ht xfs  # 显示指定类型磁盘的使用情况,-t, --type=TYPE 
...
cmd > /dev/null 2>&1 &         # /dev/null是黑洞文件,写到这里的文件都会被丢弃,如果输出日志太长了就可以重定向到这里,2给1,1给黑洞
