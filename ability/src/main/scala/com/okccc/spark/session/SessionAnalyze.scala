...
//import com.okccc.spark.common.{ConfigurationManager, Constants, ParamUtils, StringUtils, UserInfo, UserVisitAction, ValidUtils}
...
// */
//object SessionAnalyze {
//
//  def main(args: Array[String]): Unit = {
//
//    // 获取任务参数(为了方便直接从配置文件中获取,企业中会从一个调度平台获取)
...
//
//    // 创建Spark配置信息
//    val conf: SparkConf = new SparkConf().setAppName("SessionAnalyze").setMaster("local[*]")
//    // 创建Spark客户端
...
//
//    // 获取用户行为RDD
//    val actionRDD: RDD[UserVisitAction] = this.getActionRDD(spark, taskParam)
//    // 结构转换
//    val sessionRDD: RDD[(String, UserVisitAction)] = actionRDD.map((item: UserVisitAction) => (item.session_id, item))
//    // 缓存数据
//    sessionRDD.cache()
//    sessionRDD.persist(StorageLevel.MEMORY_ONLY)
//    // 将数据转换为Session粒度,格式为<sessionid,(sessionid,searchKeywords,clickCategoryIds,age,professional,city,sex)>
//    val sessionid2AggrInfoRDD: RDD[(String, String)] = this.aggregateBySession(spark, sessionRDD)
//
//    // 设置自定义累加器,实现所有数据的统计功能,注意累加器也是懒执行的
//    val sessionAggrStatAccumulator: SessionAggAccumulator = new SessionAggAccumulator
//
//    // 注册自定义累加器
...
//    val sessionid2detailRDD: RDD[(String, UserVisitAction)] = getSessionid2detailRDD(filteredSessionid2AggrInfoRDD, sessionRDD)
//
//    // 对数据进行内存缓存
...
//    val top10CategoryList: Array[(CategorySortKey, String)] = getTop10Category(spark, taskUUID, sessionid2detailRDD)
//
...
//
//    // 获得所有需要求的category集合
//    val top10CategoryIdRDD: RDD[(Long, Long)] = spark.sparkContext.makeRDD(top10CategoryList.map { case (categorySortKey, line) =>
//      val categoryid: Long = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_CATEGORY_ID).toLong;
...
//    val categoryid2sessionCountRDD: RDD[(Long, String)] = sessionid2ActionsRDD.flatMap { case (sessionid, userVisitActions) =>
...
//    val top10CategorySessionCountRDD: RDD[(Long, String)] = top10CategoryIdRDD.join(categoryid2sessionCountRDD).map { case (cid, (ccid, value)) => (cid, value) }
//
//    // 第三步：分组取TopN算法实现,获取每个品类的top10活跃用户
//
//    // 先按照品类分组
//    val top10CategorySessionCountsRDD: RDD[(Long, Iterable[String])] = top10CategorySessionCountRDD.groupByKey()
//
//    // 将每一个品类的所有点击排序,取前十个,并转换为对象
//    val top10SessionObjectRDD: RDD[Top10Session] = top10CategorySessionCountsRDD.flatMap { case (categoryid, clicks) =>
//      // 先排序,然后取前10
//      val top10Sessions: List[String] = clicks.toList.sortWith(_.split(",")(1) > _.split(",")(1)).take(10)
//      // 重新整理数据
//      top10Sessions.map { case line =>
//        val sessionid: String = line.split(",")(0)
...
//      .save()
//
//    val top10SessionRDD: RDD[(String, String)] = top10SessionObjectRDD.map(item => (item.sessionid, item.sessionid))
//
//    // 第四步：获取top10活跃session的明细数据
//    val sessionDetailRDD: RDD[SessionDetail] = top10SessionRDD.join(sessionid2ActionRDD).map { case (sid, (sessionid, userVisitAction)) =>
...
//
//    // 只将点击行为过滤出来
//    val clickActionRDD: RDD[(String, UserVisitAction)] = sessionid2detailRDD.filter { case (sessionid, userVisitAction) => userVisitAction.click_category_id != null }
//
//    // 获取每种类别的点击次数
//    // map阶段：(品类ID,1L)
//    val clickCategoryIdRDD: RDD[(Long, Long)] = clickActionRDD.map { case (sessionid, userVisitAction) => (userVisitAction.click_category_id, 1L) }
//
//    // 计算各个品类的点击次数
...
//
//    // 过滤订单数据
//    val orderActionRDD: RDD[(String, UserVisitAction)] = sessionid2detailRDD.filter { case (sessionid, userVisitAction) => userVisitAction.order_category_ids != null }
//
//    // 获取每种类别的下单次数
//    val orderCategoryIdRDD: RDD[(Long, Long)] = orderActionRDD.flatMap { case (sessionid, userVisitAction) => userVisitAction.order_category_ids.split(",").map(item => (item.toLong, 1L)) }
//
//    // 计算各个品类的下单次数
...
//
//    // 过滤支付数据
//    val payActionRDD: RDD[(String, UserVisitAction)] = sessionid2detailRDD.filter { case (sessionid, userVisitAction) => userVisitAction.pay_category_ids != null }
//
//    // 获取每种类别的支付次数
//    val payCategoryIdRDD: RDD[(Long, Long)] = payActionRDD.flatMap { case (sessionid, userVisitAction) => userVisitAction.pay_category_ids.split(",").map(item => (item.toLong, 1L)) }
//
//    // 计算各个品类的支付次数
...
//    val clickJoinRDD: RDD[(Long, String)] = categoryidRDD.leftOuterJoin(clickCategoryId2CountRDD).map { case (categoryid, (cid, optionValue)) =>
//      val clickCount: Long = if (optionValue.isDefined) optionValue.get else 0L
//      val value: String = Constants.FIELD_CATEGORY_ID + "=" + categoryid + "|" + Constants.FIELD_CLICK_COUNT + "=" + clickCount
//      (categoryid, value)
//    }
//
//    // 将所有品类信息与订单次数信息结合【左连接】
//    val orderJoinRDD: RDD[(Long, String)] = clickJoinRDD.leftOuterJoin(orderCategoryId2CountRDD).map { case (categoryid, (ovalue, optionValue)) =>
//      val orderCount: Long = if (optionValue.isDefined) optionValue.get else 0L
//      val value: String = ovalue + "|" + Constants.FIELD_ORDER_COUNT + "=" + orderCount
//      (categoryid, value)
//    }
//
//    // 将所有品类信息与付款次数信息结合【左连接】
//    val payJoinRDD: RDD[(Long, String)] = orderJoinRDD.leftOuterJoin(payCategoryId2CountRDD).map { case (categoryid, (ovalue, optionValue)) =>
//      val payCount: Long = if (optionValue.isDefined) optionValue.get else 0L
//      val value: String = ovalue + "|" + Constants.FIELD_PAY_COUNT + "=" + payCount
//      (categoryid, value)
//    }
//
//    payJoinRDD
...
//    val sortKey2countRDD: RDD[(CategorySortKey, String)] = categoryid2countRDD.map { case (categoryid, line) =>
//      val clickCount: Long = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_CLICK_COUNT).toLong
//      val orderCount: Long = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_ORDER_COUNT).toLong
//      val payCount: Long = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_PAY_COUNT).toLong
...
//    val time2sessionidRDD: RDD[(String, String)] = sessionid2AggrInfoRDD.map { case (sessionid, aggrInfo) =>
...
//    val dateHourExtractMap: mutable.HashMap[String, mutable.HashMap[String, ListBuffer[Int]]] = mutable.HashMap[String, mutable.HashMap[String, mutable.ListBuffer[Int]]]()
...
//          val starttime: String = StringUtils.getFieldFromConcatString(sessionAggrInfo, "\\|", Constants.FIELD_START_TIME)
//          val searchKeywords: String = StringUtils.getFieldFromConcatString(sessionAggrInfo, "\\|", Constants.FIELD_SEARCH_KEYWORDS)
//          val clickCategoryIds: String = StringUtils.getFieldFromConcatString(sessionAggrInfo, "\\|", Constants.FIELD_CLICK_CATEGORY_IDS)
...
//    val sessionDetailRDD: RDD[SessionDetail] = extractSessionDetailRDD.map { case (sid, (sessionid, userVisitAction)) =>
...
//    // 从Accumulator统计串中获取值
//    val session_count: Double = value(Constants.SESSION_COUNT).toDouble
//
...
//
//    // 将统计结果封装为Domain对象
//    val sessionAggrStat: SessionAggStat = SessionAggStat(taskUUID,
...
//    val sessionAggrStatRDD: RDD[SessionAggStat] = spark.sparkContext.makeRDD(Array(sessionAggrStat))
...
//
//    var _parameter: String = (if (startAge != null) Constants.PARAM_START_AGE + "=" + startAge + "|" else "") +
...
//
//    // 根据筛选参数进行过滤
//    val filteredSessionid2AggrInfoRDD: RDD[(String, String)] = sessionid2AggrInfoRDD.filter { case (sessionid, aggrInfo) =>
//      // 接着,依次按照筛选条件进行过滤
...
//    val userid2PartAggrInfoRDD: RDD[(Long, String)] = sessionid2ActionsRDD.map { case (sessionid, userVisitActions) =>
//
...
//
//      // 遍历session所有的访问行为
//      userVisitActions.foreach { userVisitAction: UserVisitAction =>
//        if (userid == -1L) {
//          userid = userVisitAction.user_id
//        }
//        val searchKeyword: String = userVisitAction.search_keyword
//        val clickCategoryId: Long = userVisitAction.click_category_id
//
//        // 实际上这里要对数据说明一下
...
//    val userid2InfoRDD: RDD[(Long, UserInfo)] = spark.sql("select * from user_info").as[UserInfo].rdd.map(item => (item.user_id, item))
//
...
//   * 获取指定日期范围内对象的用户行为数据
//   */
//  def getActionRDD(spark: SparkSession, taskParam: JSONObject): RDD[UserVisitAction] = {
//    // 从任务参数解析时间字段
