...
object DauAPP {
  def main(args: Array[String]): Unit = {
    ...
    // 配置参数优先级：SparkConf(代码写死) > com.okccc.spark-submit(动态指定) > com.okccc.spark-defaults.conf(集群配置)
    val conf: SparkConf = new SparkConf()
      .setAppName("nginx-kafka-com.okccc.spark-es/redis")
      .set("com.okccc.spark.serializer", "org.apache.com.okccc.spark.serializer.KryoSerializer")
...
//    jsonDStream.print()  // {"mid":"11","hr":"18","dt":"2021-01-16","uid":"11","en":"start"...}
    ...
    // 实时计算中去重是常见需求,redis/mysql(性能一般)/com.okccc.spark updateStateByKey(checkpoint小文件问题)
