 ...
 * Desc: 恶意登录检测,短时间内连续多次登录失败就认定为恶意登录,flink提供了cep(complex event processing)专门针对连续多次这种复杂事件处理
 */
...
//    prop.put("value.deserializer", classOf[StringDeserializer])
    ...
    // 使用CEP进行登录校验
    // 1).定义pattern：5秒内有3次登录失败事件,连续事件用.next不连续事件用.followed By
    ...
    // 3).获取符合pattern的事件序列
    val resultStream: DataStream[LoginFailWarning] = patternStream.select(new LoginFailCheckByCEP())
 ...
 */
class LoginFailCheckByCEP() extends PatternSelectFunction[LoginEvent, LoginFailWarning] {
  // pattern匹配到的事件序列存在util.Map<String(事件名称), util.List(事件序列)>
