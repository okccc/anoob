...
# mapping
# 用于定义文档中的字段类型,创建索引时如果不设定mapping,系统会自动推断字段类型
GET ${index_name}/_mapping  # 查看索引的mapping
...
POST _reindex               # 索引拷贝,_reindex可以将一个索引的快照数据copy到另一个索引
