...
# 消息队列两种模式
1.点对点模式(一对一)：一个消息只能被一个消费者消费,消费完就从queue移除
...
broker：一个kafka节点就是一个broker,集群由多个broker组成,一个broker可以容纳多个topic
...
# kafka高低阶消费者？
基于receiver的方式,使用kafka高阶api在zk中保存消费过的offset,配合wa机制可以保证数据零丢失,但是数据可能不止一次被消费,因为spark和zk是不同步的
