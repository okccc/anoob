...
select shop,date_sub(date, rn),count(1)
from(
    select shop,date,row_number() over(partition by shop order by date) rn
    from sale
    where money > 0
    ) t1
group by shop,date_sub(date, rn)
having count(1) >= 3;
...
u3     2017-01-22    8
-- 统计每个用户累计访问次数
select uid,mn,mn_cnt,sum(mn_cnt) over(partition by uid order by mn) as total_cnt
from(
    select uid,mn,sum(cnt) as mn_cnt
    from(
        select uid,substr(date,1,7) as mn,cnt from action
        ) t1
    group by uid,mn
    ) t2
group by uid,mn,mn_cnt;
-- 统计结果
uid    mn    mn_cnt    total_cnt
u1    2017-01  11     11
u1    2017-02  12     23
u1    2017-03  25     48
...
from(
    select shop,uid,count(1) as cnt from visit group by shop,uid
    ) t1
where rn <= 3;

-- 4.订单数据
oid    uid    money    date
o1     u1     300     2020-01-01
o2     u2     600     2020-01-05
-- 统计2020年每个月的订单数、用户数、总成交额
select substr(date,1,7),count(oid),count(distinct uid),sum(money) from orders group by substr(date,1,7);
-- 统计5月份的新客数(5月份才有第一笔订单)
select uid,min(date) from orders group by uid having substr(min(date),1,7) = '2020-05';

-- 有如下三张表
create table student(id int comment '学号', name string comment '姓名', age int comment '年龄');
create table course(cid string comment '课程号', cname string comment '课程名');
create table grade(id int comment '学号', cid string comment '课程号', score int comment '成绩');
-- 1.查询平均成绩大于60分的学生的姓名、年龄、平均成绩
select s.name,s.age,g.avg_score
from student s
left join 
(select id,avg(score) avg_score from grade group by id having avg_score > 60) g on s.id = g.id
-- 2.查询没有'001'课程成绩的学生的姓名、年龄

-- 3.查询有'001'\'002'这两门课程下,成绩排名前3的学生的姓名、年龄
