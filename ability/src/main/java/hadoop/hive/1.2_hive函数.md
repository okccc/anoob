...
select s.stat_date,  
       ...
       count(distinct case when datediff(t.stat_date, s.stat_date) = 7 then s.imei end) as "七日留存人数"
from  
...
inner join
    (
        select g1.id,sum(g1.score) sum_score
        from 
             grade g1
        inner join
            (select id,count(1) from grade where cid='001' or cid='002' group by id having count(1)=2) g2 on g1.id = g2.id 
        group by g1.id
...
```sql
-- team表只有一个字段name,有4条记录a/b/c/d对应4个球队,两两之间比赛列出所有可能组合
select a.name,b.name from team a, team b where a.name < b.name;
```

```sql
-- 删除学生表中除了id其它都相同的冗余信息
delete from student where id not in (select min(id) from student group by name,age,course,score);
```

```sql
-- 销售额如下
shop  month  amount     shop    m1    m2    m3
s1    01     50万        s1    50万   60万   40万
s1    02     60万        s1    44万   37万   52万
s2    01     55万        ...
-- 将店铺每个月销售额放到同一行的多个列
select shop,
       (select amount from sale s1 where month = 01 and s.shop=s1.shop) as m1,
       (select amount from sale s2 where month = 02 and s.shop=s2.shop) as m2,
       (select amount from sale s3 where month = 03 and s.shop=s3.shop) as m3
from sale s
group by shop;
```