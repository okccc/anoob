...
topic
# 消息队列/消息分类,一个topic分成多个partition,分区是为了让消费者并行处理,分区内部消息有序先进先出,每个消息都有offset
# topic是逻辑的,partition是物理的,一个partition只能在一个broker上,一个broker可以有多个partition
# producer往partition写数据时会指定消息的key-value,按照key的hash值取余0/1/2进行分配,如果没有key就轮询
# consumer可以消费同一个broker上的多个partition,在一个消费者组中partition只会被消费一次,但是不同的消费者组可以消费同一个partition,相互之间互不干扰
partition
