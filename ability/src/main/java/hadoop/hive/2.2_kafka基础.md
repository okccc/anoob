...
# 启动kafka
[root@master1 ~]# cd /opt/module/kafka_2.11-0.11.0.2/bin -> vim start-kafka.sh -> chmod +x start-kafka.sh
nohup ./kafka-server-start.sh ../config/server.properties > ../logs/kafka.log 2>&1 &

# 查看topic列表/详细信息
[root@master1 ~]# bin/kafka-topics.sh --zookeeper cdh1:2181,cdh2:2181,cdh3:2181 --list
[root@master1 ~]# bin/kafka-topics.sh --zookeeper cdh1:2181,cdh2:2181,cdh3:2181 --describe --topic t01

# 创建/删除/修改 topic
[root@master1 ~]# bin/kafka-topics.sh --zookeeper cdh1:2181 --create --topic t01 --partitions 3 --replication-factor 3
[root@master1 ~]# bin/kafka-topics.sh --zookeeper cdh1:2181 --delete --topic t01
[root@master1 ~]# bin/kafka-topics.sh --zookeeper cdh1:2181 --alter --topic t01 --partitions 2

# 在zookeeper中查看/删除,kafka0.8版本是zk保存offset,kafka0.10版本是自己保存offset,数据是放在broker节点
[zk: localhost:2181(CONNECTED) 0] ls /brokers | rmr /brokers/topics/t01

# 生产者/消费者
[root@master1 ~]# bin/kafka-console-producer.sh --broker-list cdh1:9092,cdh2:9092,cdh3:9092 --topic t01
[root@master1 ~]# bin/kafka-console-consumer.sh --bootstrap-server cdh1:9092 --from-beginning --topic t01

# 查看consumer group列表/详细信息
[root@master1 ~]# bin/kafka-consumer-groups.sh --bootstrap-server cdh1:9092,cdh2:9092,cdh3:9092 --list
[root@master1 ~]# bin/kafka-consumer-groups.sh --bootstrap-server cdh1:9092,cdh2:9092,cdh3:9092 --describe --group g01
```
...
producer
# 生产者,将message发送到指定topic的partition,消息是通信的基本单位
consumer
# 消费者,从topic中pull数据,kafka依赖zookeeper在consumer端做负载均衡
consumer group
# 消费者组,由多个消费者组成,每个消费者负责消费不同分区的数据,可用于扩展和容错,消费者组是逻辑上的订阅者
broker
# 节点,负责处理消息的读写请求以及存储消息,kafka集群由多个broker组成,broker本身没有主从,依赖zookeeper做协调服务,一个broker可以容纳多个topic
topic  
# 消息队列/分类,一个topic分成多个partition,分区是为了让消费者并行处理,分区内部消息严格有序先进先出,每个消息都有个序号叫offset,topic是逻辑的partition是物理的,一个partition只能在一个broker上,一个broker可以有多个partition
# 生产者往partition写消息时会指定消息的key-value,具体写进哪个partition可以按照key的hash值取余0/1/2进行分配,如果没有key就按照轮询的负载均衡方式
# 消费者可以消费同一个broker上的多个partition,在一个消费者组中partition只会被消费一次,但是不同的消费者组可以消费同一个partition,相互之间互不干扰
partition
# 分区,每个partition对应一个log文件,producer生产的数据会不断追加到log文件末尾,且每条数据都有自己的offset便于定位,offset保存在kafka的内置topic __consumer_offsets
segment
# 片段,为了防止log文件过大导致数据难定位,kafka采用分片和索引机制,将partition分为多个segment,每个segment对应.index(存储索引信息,相当于key)和.log(存储数据,相当于value)两个文件,index和log文件以当前segment第一条消息的offset命名
replication
# 副本,为了保证HA,每个partition都有副本,一个leader(生产者发送数据和消费者消费数据的对象)和多个follower(从leader同步数据,leader故障时会选取某个follower成为新的leader)
offset
...
receive-based approach：offset存储在zookeeper,由receiver维护,spark获取数据存入executor中,调用kafka高阶api
direct approach：offset存储在zookeeper,由spark维护,且可以从每个分区读取数据,调用kafka低阶api
