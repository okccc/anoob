...
# 数据仓库分层架构
ods(数据采集)：原始数据层
dwd(格式化)：数据明细层,将ods层数据进行数据清洗(空值/错误值/极限值)/改变存储格式(orc/parquet)/数据脱敏(udf),etl工具可以是sql/python/mr/rdd等
...
事实表：存储指向各个维度的外键值及事实的度量(measure),一行对应一个可度量事件,一般是整数型或浮点型,比如订单表
维度表：存储该维度的元数据信息,统计分析时可考量的角度,比如用户维/商家维/商品维/地区维/时间维

星型模型：每个维度表直接与事实表连接,不存在渐变维度,所以数据有一定冗余,但是查询时join的表更少,数据仓库查询性能更重要,磁盘空间不是问题,以空间换时间,提高效率
雪花模型：存在一个或多个维度表不直接连接事实表,而是通过其它维度表连接到事实表,将星型模型的维度进一步细分,消除数据冗余减少存储的数据量,但是设计更复杂查询效率较低
星型模型和雪花模型都是基于一个事实表,星座模型是基于多个事实表,也是数据仓库的常态,和前两个模型不冲突,生产环境通常是混合着用
