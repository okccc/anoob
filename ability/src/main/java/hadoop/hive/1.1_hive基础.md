...
-- 内部表：默认路径hive.metastore.warehouse.dir=/user/hive/warehouse,数据由hive自己管理,删表会直接删除hive元数据metadata以及hdfs文件
...
lines terminated by '\n'  -- 行分隔符
-- orc将数据按行分块按列存储,保证同一条记录在一个块上,snappy压缩率1:10左右节省空间,sqoop从mysql抽过来的表只能是textfile格式
stored as orc tblproperties ("orc.compress"="snappy")
-- 外部表：手动指定location,数据由hdfs管理,删表只会删除hive元数据metadata,hdfs文件还在
...
### 数据仓库
```bash
# 数据仓库四大特性：面向主题、集成的、稳定的、历史数据随时间动态变化
# 大数据平台的数据源主要来自日志数据,大数据框架解决日志数据的采集、清洗、存储、离线/实时分析
# 数据仓库 - 为公司各部门提供统一的数据出口
# 产品信息分析 - 新增用户/活跃用户/沉默用户/启动次数/版本统计/留存率/新鲜度 
# 用户行为分析 - 建立用户画像 - 精准广告推送/精准商品推荐
# 人工智能基础 - 提供大数据量的支撑,近些年硬件发展也很快
