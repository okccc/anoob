...
解决：SequenceFile和CombineFileInputFormat进行小文件合并操作

# HDFS安全模式？
启动hadoop集群时,启动了namenode未启动datanode,此时文件系统处于安全模式,无法更新和删除文件,只能查看有哪些文件而不能查看文件内容,因为datanode还未启动
namenode会等待datanode向它发送块报告,接收到的datanode blocks和total blocks达到99.9%表示块数量一致,文件系统会在等待30秒之后会退出安全模式
```
