...
# 存储和压缩
textfile(hadoop默认)和sequencefile基于行存储不常用,orc和parquet基于列存储并且可以手动安装snappy和lzo进行压缩(常用)
orc读取行数更少查询更快,但是parquet支持hadoop所有项目,snappy压缩效率更高但是不支持文件分割,如果压缩文件很大可能会出现map端读取文件时的数据倾斜
所以实际生产中parquet存储lzo压缩更常见,可以避免读取不可分割大文件引发的数据倾斜,但是没有超大文件时orc存储snappy压缩效率还是非常高的
...
-- 3.小文件处理,使用SequenceFile(流计算场景)/CombineFileInputFormat代替TextFile、减少reduce数量、shell脚本合并小文件
-- 4.sql语句采用谓词下推技术,提早过滤数据减少数据传输
-- 5.jvm重用,适用于小文件和task过多的场景,大多数执行时间很短,jvm启动消耗很大,jvm重用可以使jvm实例在一个job中重复使用多次,修改mapred-site.xml
-- 6.选择tez作为计算引擎,mr会将迭代任务的中间结果多次写入hdfs,而tez可以将有多个依赖的job转换为一个job,这样只需写一次hdfs从而大大提升计算性能
-- 7.本地模式,适用于处理数据量很小的情况,mr启动过程消耗的时间可能比实际计算时间还长,单节点完全够用,set hive.exec.mode.local.auto=true
-- 8.提高并行执行的任务数,set hive.exec.parallel=true; set hive.exec.parallel.thread.number=16;
-- 9.设置合理的map/reduce数量
-- 10.数据倾斜问题
```
...
set hive.groupby.skewindata=true;  -- 只针对单列有效,会生成两个mrjob,第一个job中map输出结果会随机分配到reduce,解决key分布不均匀的问题,第二个job再按key分组聚合  

-- 3.reduce数量太少导致,比如count distinct属于全局操作,最终会用一个reduce处理导致耗时过长,可以先group by去重,虽然会多一个job但是数据量很大时是值得的
